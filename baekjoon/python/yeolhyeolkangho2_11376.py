...
def solution(g: defaultdict, n: int, m: int):
    return dinic(g, 0, n+m+1)
...
def main():
    ...
    for e in range(1, n+1):
        works = read_list_int()
        for work in works[1:]:
            add_connections(g, e, work+n)

    make_source_to_first_group_as_childs(g, 0, 1, n)
    make_second_group_to_sink(g, n+m+1, n+1, n+m)
